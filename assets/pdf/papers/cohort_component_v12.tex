...
Many applied forecasting problems share a common structure: a population
of interest evolves over time through entries and exits, and the analyst
must project its future size and composition. Labour force planning,
professional licensing registries, pension fund membership, and customer
portfolio management are examples of domains where this structure appears
naturally. The cohort component method is the standard tool for such
settings. It accounts for the population dynamics explicitly---tracking
inflow, outflow, and the surviving stock period by period---rather than
fitting a trend line to aggregate counts.

Despite its practical ubiquity, the cohort component method is rarely
connected to the mainstream statistical theory of time series. This note
establishes that connection. The central observation is simple: under
constant attrition and inflow rates, the scalar cohort component recursion
is exactly an AR(1) process with drift, and the multi-age-group
generalisation is a VAR(1) model. This is not an approximation; it is an
algebraic identity. The consequence is that the entire toolkit of linear
time-series analysis---estimation, inference, forecasting intervals,
impulse response analysis---applies directly to cohort component models,
without modification.

The note is organised as follows. Sections~\ref{sec:snapshots}--\ref{sec:ar1}
develop the cohort component model from first principles, derive the AR(1)
and VAR(1) representations, and study the long-run equilibrium and
stationary decomposition. Section~\ref{sec:curvefitting} addresses the
auxiliary problem of estimating age profiles from historical cohort data,
which is a prerequisite for calibrating the model parameters. Finally,
Section~\ref{sec:forecast} lays the statistical foundation for the
forecasting step itself, establishing what ``predicting a future
observation'' means rigorously in the simplest i.i.d.\ setting as a
precursor to the time-series case.
...
This note has established two main results. First, the cohort component
method is not merely an accounting device: under constant rates, it
reduces exactly to an AR(1) or VAR(1) linear time-series model. The
long-run equilibrium size $I/r^O$ is the fixed point of the AR(1)
recursion, and the rate of convergence is governed by $1-r^O$. The
multi-age-group generalisation produces the companion matrix structure
familiar from VAR analysis.

Second, the statistical foundation of point prediction in the i.i.d.\
setting has been developed in full. The key result
(Proposition~\ref{prop:decomp}) shows that predictive MSE and estimation
MSE are related by the identity
$\mathrm{MSE}_\mathrm{pred}(f) = \mathrm{MSE}_\mathrm{est}(f) + \mathrm{Var}(Y)$,
valid for every statistic $f$. The variance term is the irreducible noise
floor: no predictor can eliminate it. As a consequence, the best predictor
and the best estimator of $\E Y$ are always the same statistic, although
their performance values differ by exactly $\mathrm{Var}(Y)$.

Several directions are planned for subsequent notes. The most immediate is
to replace the i.i.d.\ assumption with stationarity. In the stationary
AR(1) setting, $Y_{n+1}$ is no longer independent of the sample; the
optimal predictor $\E[Y_{n+1} \mid Y_1,\ldots,Y_n]$ becomes a non-trivial
linear function of the observations, and the noise floor is replaced by
the conditional variance $\mathrm{Var}(Y_{n+1} \mid Y_1,\ldots,Y_n)$,
which can be reduced by using the observed history. This connects the
forecast foundations directly to the AR(1) model derived in the first part
of the note. A further direction is to extend the VAR(1) analysis to
include time-varying rates, seasonal effects, and covariate-driven inflow,
which arise naturally in applications.
